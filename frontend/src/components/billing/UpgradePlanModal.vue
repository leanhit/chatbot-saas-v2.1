<template>
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white dark:bg-gray-800 rounded-lg max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
      ...
      <!-- Current Plan Info -->
      <div class="p-6 bg-gray-50 dark:bg-gray-700 rounded-lg m-6">
        <div class="flex justify-between items-center">
          <div>
          ...
          <div class="p-3 bg-blue-100 dark:bg-blue-900/20 rounded-lg">
            <Icon :icon="getPlanIcon(currentPlan.type)" class="text-blue-600 dark:text-blue-400 text-2xl" />
          </div>
        </div>
      </div>
      ...
      <div class="px-6 pb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          >
            <!-- Current Plan Badge -->
            <div v-if="plan.id === currentPlan.id" class="absolute -top-3 left-1/2 transform -translate-x-1/2">
            ...
            <!-- Popular Badge -->
            <div v-if="plan.isPopular && plan.id !== currentPlan.id" class="absolute -top-3 left-1/2 transform -translate-x-1/2">
            ...
            <div class="text-center mb-6">
              ...
              <!-- Savings -->
              <div v-if="getSavings(plan) > 0" class="text-green-600 dark:text-green-400 text-sm">
            ...
            <div class="space-y-3">
              ...
              >
                <Icon 
                  :icon="getFeatureIcon(feature, currentPlan.features)" 
                  :class="getFeatureIconClass(feature, currentPlan.features)"
                  class="mt-0.5"
                />
                <div>
                  <p :class="getFeatureTextClass(feature, currentPlan.features)">
                    {{ feature.name }}
                  </p>
            ...
            >
              {{ selectedPlan?.id === plan.id ? 'Selected' : 'Upgrade to ' + plan.name }}
            </button>
            
            <div v-else class="w-full mt-6 p-3 bg-gray-100 dark:bg-gray-700 rounded-lg text-center">
      ...
      <!-- Upgrade Summary -->
      <div v-if="selectedPlan && selectedPlan.id !== currentPlan.id" class="p-6 border-t border-gray-200 dark:border-gray-700">
        <h4 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">
          Upgrade Summary
        </h4>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <!-- Changes Summary -->
          <div class="space-y-3">
            <h5 class="font-medium text-gray-900 dark:text-gray-100 mb-3">What Changes:</h5>
            <div class="space-y-2">
              <div 
              ...
              >
                ...
                <span :class="change.type === 'upgrade' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'">
          ...
          <!-- Cost Summary -->
          <div class="space-y-3">
            <h5 class="font-medium text-gray-900 dark:text-gray-100 mb-3">Cost Impact:</h5>
            <div class="space-y-2 text-sm">
              ...
              <div class="flex justify-between">
                ...
                <span :class="getPriceDifferenceClass(selectedPlan)" class="font-bold">
                  {{ getPriceDifferencePrefix(selectedPlan) }}${{ getPriceDifference(selectedPlan) }}/month
                </span>
              </div>
              <div v-if="getAnnualSavings(selectedPlan) > 0" class="flex justify-between">
        ...
        <div class="mt-4 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
          <div class="flex items-center gap-2">
            <Icon icon="ic:baseline-info" class="text-blue-600 dark:text-blue-400" />
            <div class="text-sm text-blue-800 dark:text-blue-200">
              <p class="font-medium">Effective Date:</p>
              <p>Changes will take effect immediately upon confirmation</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="p-6 border-t border-gray-200 dark:border-gray-700">
        <div class="flex gap-3">
          ...
          >
            <Icon v-if="isUpgrading" icon="ic:baseline-refresh" class="animate-spin" />
...
// Available upgrade options (mock data)
...
const canUpgrade = computed(() => {
  return selectedPlan.value && 
         selectedPlan.value.id !== props.currentPlan.id &&
         !isUpgrading.value
})
...
const getSavings = (plan) => {
  if (plan.id === props.currentPlan.id) return 0
  const currentPrice = props.currentPlan.monthlyPrice / 100
  const newPrice = plan.monthlyPrice / 100
  return Math.max(0, currentPrice - newPrice).toFixed(2)
}
...
const getPlanChanges = (newPlan) => {
  ...
  newPlan.features.forEach(newFeature => {
    const currentFeature = props.currentPlan.features.find(f => f.name === newFeature.name)
...
const getPriceDifference = (plan) => {
  if (plan.id === props.currentPlan.id) return '0.00'
  const currentPrice = props.currentPlan.monthlyPrice / 100
  const newPrice = plan.monthlyPrice / 100
...
const getPriceDifferenceClass = (plan) => {
  if (plan.id === props.currentPlan.id) return 'text-gray-600'
  const currentPrice = props.currentPlan.monthlyPrice / 100
  const newPrice = plan.monthlyPrice / 100
...
const getAnnualSavings = (plan) => {
  if (plan.id === props.currentPlan.id) return 0
  // Calculate annual savings based on yearly pricing
  const currentAnnual = props.currentPlan.annualPrice || (props.currentPlan.monthlyPrice * 12)
  const newAnnual = plan.annualPrice || (plan.monthlyPrice * 12)
  return Math.max(0, currentAnnual - newAnnual) / 100
}
...
const confirmUpgrade = async () => {
  ...
  try {
    ...
    const result = await billingStore.upgradeSubscription(
    ...
    emit('upgraded', {
      ...result,
      previousPlan: props.currentPlan,
...
.plan-card:hover:not(.opacity-50) {
