...
const customerApi = {
  ...
  async getCustomers(params = {}) {
    ...
    const response = await axios.get('/api/odoo/customers', {
  ...
  async getCustomerByPsid(psid) {
    const response = await axios.get(`/api/odoo/customers/${psid}`)
    return response.data
  },

  // Search customers
  async searchCustomers(keyword, page = 0, size = 20) {
    const response = await axios.get('/api/odoo/customers/search', {
  ...
  async getCustomersByStatus(status, page = 0, size = 20) {
    const response = await axios.get(`/api/odoo/customers/status/${status}`, {
      params: { page, size }
    })
    return response.data
  },
