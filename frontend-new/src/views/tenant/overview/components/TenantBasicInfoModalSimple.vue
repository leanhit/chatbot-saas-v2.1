    ...
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white dark:bg-gray-800">
      ...
      <div class="mb-6">
        ...
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
          {{ $t('tenant.overview.updateBasicInfo') }}
        </p>
      </div>
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4">
        <div>
          ...
          <input
            ...
            :placeholder="$t('tenant.overview.tenantName')"
          />
        </div>
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            {{ $t('tenant.overview.contactEmail') }}
          </label>
          <input
            v-model="form.contactEmail"
            type="email"
            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-primary focus:border-primary dark:bg-gray-700 dark:text-white"
            :placeholder="$t('tenant.overview.contactEmail')"
          />
        </div>

        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            {{ $t('tenant.overview.contactPhone') }}
          </label>
          <input
            v-model="form.contactPhone"
            type="tel"
            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-primary focus:border-primary dark:bg-gray-700 dark:text-white"
            :placeholder="$t('tenant.overview.contactPhone')"
          />
        </div>
...
export default {
  name: 'TenantBasicInfoModalSimple',
  props: {
    visible: {
      type: Boolean,
      default: false
