  ...
  export default {
    ...
    methods: {
      ...
      handleAvatarError(event) {
        ...
        console.error('Image classes:', img.className);
        ...
        if (originalSrc && originalSrc.includes('cwsv.truyenthongviet.vn:9000')) {
          ...
          console.log('Setting default avatar for non-Botpress error');
          img.src = require("@/assets/img/user.jpg");
        }
      },
      ...
      handleTenantGateway() {
        console.log('Header: Tenant gateway button clicked');
        console.log('Current route:', this.$router.currentRoute.value);
        
        // Navigate to tenant gateway
        this.$router.push('/tenant-gateway').then(() => {
          console.log('Navigation to tenant-gateway successful');
        }).catch(error => {
          console.error('Navigation to tenant-gateway failed:', error);
        });
