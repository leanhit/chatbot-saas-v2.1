...
import com.chatbot.integrations.image.fileMetadata.dto.FileResponseDTO;
import com.chatbot.integrations.image.fileMetadata.model.FileMetadata;
import com.chatbot.integrations.image.fileMetadata.repository.FileMetadataRepository;
...
public class FileMetadataService {
    ...
    public boolean deleteFileById(String id) {
        ...
        if (tenantId == null) {
            throw new RuntimeException("Không tìm thấy tenant ID trong context khi xóa file");
        }
        
        return fileMetadataRepository.findByIdAndTenantId(uuid, tenantId).map(file -> {
            try {
    ...
    public FileResponseDTO updateFile(String id, FileRequestDTO requestDTO) {
        ...
        Long tenantId = TenantContext.getTenantId();
        if (tenantId == null) {
            throw new RuntimeException("Không tìm thấy tenant ID trong context khi cập nhật file");
        }
    ...
    public FileResponseDTO updateFileContent(String id, FileRequestDTO requestDTO) throws Exception {
        ...
        Long tenantId = TenantContext.getTenantId();
        if (tenantId == null) {
            throw new RuntimeException("Không tìm thấy tenant ID trong context khi cập nhật nội dung file");
        }
        ...
        return fileMetadataRepository.findByIdAndTenantId(uuid, tenantId).map(file -> {
            try {
                ...
                if (requestDTO.getFiles() != null && !requestDTO.getFiles().isEmpty()) {
                    ...
                    String fileUrl = String.format("/%s/%s", bucketName, uniqueFileName);
