package com.chatbot.spokes.facebook.webhook.processor;
...
/**
 * Service that processes Facebook webhook events with full business logic.
 * Independent of Kafka execution so it can be used for both Kafka consumer
 * and synchronous fallback processing.
 */
@Service("facebookEventProcessor")
@Slf4j
@RequiredArgsConstructor
public class FacebookEventProcessor {
    ...
    public boolean tryDedup(String mid) {
    ...
    public void removeDedup(String mid) {
        if (mid == null) return;
        try {
            String key = "facebook:dedup:mid:" + mid;
            redisTemplate.delete(key);
            dedupCache.invalidate(mid);
            log.info("🔄 Cleared deduplication key for mid={} to allow retry", mid);
        } catch (Exception e) {
    ...
    public String extractMid(WebhookRequest.Messaging messaging) {
    ...
    /**
     * Process Facebook event directly
     */
    public void processEvent(FacebookKafkaEvent event) {
        ...
        try {
            ...
            // Clear dedup key on exception so retry can re-attempt processing
            String mid = extractMid(event.getMessaging());
            if (mid != null) {
                removeDedup(mid);
            }
            throw new RuntimeException("Error processing Facebook event", e);
        } finally {
